//var get_events_url = "http://192.168.2.2/~kjlmfe/slimevent/m/find?";
...
var item_template_sameday = "" 
  + "<li>"
  +   "<h2><a target='_blank' href='/event/{eid}?mobile=1'>[{category}] {title}</a></h2>"
  +   "<div class='info'>"
  +   "<ul>"
  +     "<li>时间: {begin_date} ({begin_weekday}) {begin_time} - {end_time} ({begin_remain_day})</li>"
  +     "<li>地点: {region} {addr}</li>"
  +     "<li>发起: <a target='_blank' href='/user/{organizer_id}?mobile=1'>{organizer}</a></li> "
  +   "</ul>"
  +   "</div>"
  + "</li>";
...
var item_template_diffday = "" 
  ...
  +   "<div class='info'>"
  +   "<ul>"
  +     "<li>开始时间: {begin_date} ({begin_weekday}) {begin_time} ({begin_remain_day})</li>"
  +     "<li>结束时间: {end_date} ({end_weekday}) {end_time} ({end_remain_day})</li>"
  +     "<li>地点: {region} {addr}</li>"
  +     "<li>发起: <a target='_blank' href='/user/{organizer_id}?mobile=1'>{organizer}</a></li> "
  +   "</ul>"
  +   "</div>"
